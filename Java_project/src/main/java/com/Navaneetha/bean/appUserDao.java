...
public class appUserDao {
	...
	public static int save(appuser u) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into appointment(patient_fname,patient_lname,patient_gender,patient_age,patient_mblno,patient_email,appointment_type,appointment_date,appointment_time,patient_problem,patient_address,doctor_id) values(?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	// get records from both appointment and doctors by using foreign key "doctor_id"
		public static boolean getRecordByIdForJoin(appuser u) {
			...
			try{  
				...
				PreparedStatement ps=con.prepareStatement(  
			"select * from appointment,doctor where appointment_id=?");  
