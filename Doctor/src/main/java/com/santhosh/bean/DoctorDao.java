...
public class DoctorDao {
	...
	public static int save(Doctor u) {
		...
		try {
			...
			PreparedStatement ps=con.prepareStatement("insert into doctor(d_Name,d_Role,d_Mblno,d_Email,d_Address) values(?,?,?,?,?)");
			ps.setString(1, u.getd_Name());
			ps.setString(2, u.getd_Role());
			ps.setString(3, u.getd_Mblno());
			ps.setString(4, u.getd_Email());
			ps.setString(5, u.getd_Address());
	...
	public static List<Doctor> getAllRecords(){  
		    ...
		    try{  
		        ...
		        while(rs.next()){  
		            ...
		            u.setd_Role(rs.getString("d_Role"));  
	...
	public static int update(Doctor u){  
		...
		"update doctor set d_Name=?,d_Role=?,d_Mblno=?,d_Email=?,d_Address=? where d_Id=?");  
		        ps.setString(1,u.getd_Name());  
		        ps.setString(2,u.getd_Role());  
		        ps.setString(3,u.getd_Mblno());  
		        ps.setString(4,u.getd_Email());  
		        ps.setString(5,u.getd_Address());  
		        ps.setInt(6,u.getd_Id());  
		        status=ps.executeUpdate();  
		    }catch(Exception e){System.out.println(e);}  
		    return status;  
		}  
